...
- [x] Default features (`simd-thumbnails`, `remote`, `candle`, `onnx`) no longer pull in OpenSSL. `hf-hub` and `mistralrs` are optional and use a statically vendored OpenSSL when enabled.
- [ ] TLS stack split: `hf-hub`/`mistralrs` use `native-tls` (static OpenSSL) to avoid rustls `close_notify` failures, while `reqwest` and `ort` still use `rustls-tls`. Evaluate unifying on one TLS stack (either switch `hf-hub` back to `rustls-tls` if the issue can be solved another way, or move `reqwest`/`ort` to `native-tls`).
...
- [ ] **Painless ONNX for non-technical users.** The default nightly build excludes ONNX to avoid packaging `libonnxruntime.so`. For users who just want to run taggers (e.g., WD14) without compiling from source, decide on a strategy:
  - Add an `akasha-onnx` nightly variant that bundles `libonnxruntime.so` and the binary in one tarball (possibly with a wrapper script for `LD_LIBRARY_PATH`).
  - Switch `ort` to `load-dynamic` and download the ONNX Runtime on first use.
  - Static-link ONNX Runtime (unlikely to be worth the build complexity).
  - Defer until the model-download / model-manager UI can hide the distinction from users.
