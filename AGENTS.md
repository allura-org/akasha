...
- **GPU (CubeCL) notes:** CUDA runs BF16 end-to-end (`MODEL_DTYPE` is per-backend in `src/models/burn/hydra/mod.rs`; CPU backends stay F32) with burn's `fusion` feature on. burn's `autotune` was measured SLOWER for Hydra (~0.22 vs ~0.16 s/image at F32) and adds a ~90 s un-cached warmup on first inference — do not re-enable without benchmarking. Batching is opt-in (`[models.burn] batch_size`, default 1) and still slower than singles on the reference RTX 4090 even at BF16 (0.106 vs 0.093 s/image; the GPU is launch/latency-bound, not throughput-bound). Known upstream CubeCL issues: batch ≥16 previously panicked in kernel codegen (avoided by query tiling), and no-mask fused attention materializes a padded scores workspace for unaligned `seq_kv` instead of taking a flash path. `cubecl-cuda` requires `libnccl.so` at link time (see Feature flags). CubeCL memory pools still peak at ~11 GB during an active run by design (page caching); they release on queue drain via `Model::release_memory()`. Host-side F32 weight caches/packing are skipped on GPU builds (`NEED_HOST_CACHES` in `weights.rs`).
